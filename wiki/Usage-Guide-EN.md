...
- **Prose (sentence mode)**: the second tab becomes a **Sentences** list — **one row per sentence**, split on periods (including the full-width `。`). A comma inside a sentence is not a break: `A girl with long hair, smiling, stands indoors.` is one row, not three, or the sentences would just be tags under another name. Each row is editable in place, reorderable by its drag handle and removable; a repeated sentence gets its own row rather than being merged away, so editing in this view never writes the paragraph back a sentence short. The text tab still edits the whole caption as one block, and a captioning model's paragraph lands there verbatim.
- **JSON**: one JSON document per caption. The tag views give way to a read-only highlighted JSON view and tag-level rewrites in the UI are refused — a flat list of tags cannot be assembled back into a document. These captions are edited as text, or through the assistant's JSON tools (below), which can both restructure them and edit the tags inside them.
...
**The model has to match the caption type**, and the check runs in both directions: picking a caption model while the caption type is tags (or Anima Tag) refuses the run with a hint to pick a tagger, exactly as picking a tagger under a prose type refuses it the other way. A description written into a tag caption would arrive as one long tag, and the next time the caption is read every comma inside that sentence becomes a tag boundary. A server that reports no model category (an older build) is trusted and runs either way.
...
**Batch runs follow the same split.** Under a prose type the batch dialog becomes "Batch AI description" and offers only **append** and **overwrite** ("recognize only" needs the compare view and Sheet mode needs a tag rule set; both fall back to append there, while the mode you actually picked is remembered for the next tag type). Append adds the described sentences the caption does not already carry; overwrite replaces it entirely. Threshold, ignored tags and preserved tags are tag settings and are hidden. An image the model answers nothing for is counted as failed and left exactly as it was — never emptied, and an image whose caption already carries every sentence the model returned is left byte-identical and counted as unchanged, so a re-run over a described dataset really does nothing. Selecting the wrong kind of model — a tagger under a prose type, or a caption model under a tag type — disables the start button with the same hint as the single-image run. JSON types get no batch run at all: the dialog explains why and points at the assistant's JSON caption tools, because a tag merge would flatten each document into one comma-separated line.
...
- **Edit tags in bulk**: one call carrying the rules — add tags, remove tags, rename tags — rewrites every caption in scope and undoes as one operation. Added tags can go at a position (start, end, a specific index) or beside an anchor tag, and images without a caption file get one created. The sweep narrows with the same filters the image listing takes (must have these tags, must not have those, filename query, untagged only). Matching folds case and underscore/space differences while each file keeps its own spelling, unless a rename changes it. It targets the active caption type by default, or another tag-format type by extension without switching first. The result lists every added, removed and renamed tag with how many captions it came out of.
...
- **Convert into a tag-list type in bulk** (WD14 tags ⇄ Anima Tag, and JSON → either). **Going back from JSON** is what a dataset captioned as Anima JSON needs before it can train a LoRA: give a field order and each document is flattened into a tag line in that key order. Keys left out of the order don't lose their tags — they're appended after and named in the result. The natural-language key has to be named separately, or its sentence gets comma-split into bogus tags; it becomes the trailing sentence when the target is Anima Tag, and is dropped **and counted** when the target is plain tags. The rules are given once for the whole dataset — tags to drop, tags to rename, one spelling style (underscores or spaces), whether parentheses are escaped, a priority order, and a trigger word to put first — and the tool rewrites every caption itself. Nothing is invented: every output tag comes from the source or from the trigger word. An Anima Tag description is carried over when the target is Anima Tag too, and dropped **and counted** when the target is plain tags. The result lists each dropped and renamed tag with how many captions it came out of, so the whole conversion can be checked from one answer — and a drop rule that matched nothing is visible by its absence rather than passing as "there were none". The whole batch undoes as one operation.

When a JSON-format type is configured, three more tools show up:
...
- **Edit tags in bulk**: leaves the structure alone and changes the tags themselves — **adding tags to a field, removing tags, renaming tags**. This is the only route to a dataset-wide tag rewrite on JSON captions: the ordinary tag batch tools (bulk tag editing, sorting) all refuse a JSON type, because they would flatten the document into a comma line. The rules are given once and matching ignores case and underscore/space differences; a rename that lands on a tag the field already carries merges into it instead of writing it twice, and adding a tag a field already has is not duplicated. The document's keys, their order, and every value no rule matched are kept exactly as they were — natural-language fields included: text that matches no rule comes out byte-identical, and a "skip fields" list can put prose fields like `nl` entirely out of reach. The result lists every added, removed and renamed tag with how many captions it came out of. The whole batch undoes as one operation.
