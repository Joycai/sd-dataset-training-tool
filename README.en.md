...
- **AI describe, in sentence mode**: on a prose caption the same button (`Ctrl+E`) runs a **natural-language caption model** instead; its output is split into sentences and appended to the sentence list, with no compare view — that one diffs tag sets, and a paragraph is not one. Picking a WD tagger there is refused with a hint to choose a caption model, rather than writing a tag line in as a sentence.
