...
abstract class AppLocalizations {
  ...
  /// No description provided for @captionTypeFormatAnimaTag.
  ///
  /// In en, this message translates to:
  /// **'Anima Tag'**
  String get captionTypeFormatAnimaTag;

  /// No description provided for @captionTypeFormatAnimaTagDesc.
  ///
  /// In en, this message translates to:
  ...
  /// No description provided for @sentencesTab.
  ///
  /// In en, this message translates to:
  /// **'Sentences'**
  String get sentencesTab;
  ...
  /// No description provided for @captionHintProse.
  ///
  /// In en, this message translates to:
  ...
  /// No description provided for @addSentenceHint.
  ///
  /// In en, this message translates to:
  /// **'Type a sentence and press Enter'**
  String get addSentenceHint;
  ...
  /// No description provided for @captionDescriptionLabel.
  ///
  /// In en, this message translates to:
  /// **'Description'**
  String get captionDescriptionLabel;

  /// No description provided for @captionDescriptionHint.
  ///
  /// In en, this message translates to:
  /// **'A sentence or two about the image, appended after the tags'**
  String get captionDescriptionHint;

  /// No description provided for @captionDescriptionTooltip.
  ///
  /// In en, this message translates to:
  ...
  /// No description provided for @wd14FromAnimaTagPresetTitle.
  ///
  /// In en, this message translates to:
  /// **'Anima Tag to WD14 tags'**
  String get wd14FromAnimaTagPresetTitle;

  /// No description provided for @wd14FromAnimaTagPresetBody.
  ///
  /// In en, this message translates to:
  /// **'Convert this dataset\'s Anima Tag captions back into plain WD14 tag captions for LoRA training.\n\nWD14 is a tagger, not a caption format, so there is no official field order to restore. What the trainers and the prompt parser actually require is this:\n- A flat comma-separated danbooru tag list and nothing else. The natural-language sentence is dropped, not folded into tags — the Anima Tag file keeps it, the WD14 file must not have it.\n- No @ on artist tags. That prefix is Anima\'s; a danbooru artist tag is just the name.\n- Parentheses inside a tag are escaped with backslashes: hatsune miku (racing) becomes hatsune miku \\(racing\\). Unescaped parentheses are attention syntax to the prompt parser, so an unescaped character variant silently reweights the prompt instead of naming the character.\n- One spelling style across the whole dataset — underscores everywhere or spaces everywhere, never mixed. Keep whichever style the dataset already uses.\n- Drop the Anima quality, rating and year vocabulary: newest, recent, mid, early, old, year followed by a number, safe, sensitive, nsfw, explicit, masterpiece, best quality, good quality, normal quality and the whole score_ series. Quality is added at inference time; a LoRA that learned it cannot be asked for anything else.\n- Drop meta tags if this dataset carries any: highres, absurdres, commentary, commentary request, translated, annotated, artist name, signature, watermark, username, web address.\n- The trigger word, if this dataset has one, goes first in every caption. Trainers protect the leading tokens with keep_tokens and shuffle the rest, which is also why the order of everything after it barely matters — keep the Anima ordering rather than inventing a new one.\n\nSteps:\n1. Run get_dataset_overview and say which type is the Anima Tag source and which is the tag-format target. If no tag-format type exists, stop and tell me to add one in the caption type settings first.\n2. Run get_tag_stats and show me, before the first write, two lists: every tag you are going to drop, and every tag you are going to respell (@ stripped, parentheses escaped, spelling style normalised). That list is the whole conversion — I want to check it once rather than audit hundreds of captions afterwards.\n3. Then run convert_captions_to_tags ONCE for the whole dataset: source is the Anima Tag type, target is the tag type, drop carries the list from step 2, rename maps each @artist tag to its plain name, spacing pins one spelling style, parentheses is escape, and prepend is the trigger word if I gave you one. Every rule above is dataset-wide, so this is one call — do not loop write_caption_file, do not look at the images, and do not call the tagger.\n4. Read the result back to me: written, skipped, descriptions_dropped, and the dropped_tags / renamed_tags counts. A drop rule missing from dropped_tags matched nothing — check it for a typo rather than assuming the dataset had none.\n\nTrigger word, if this dataset has one: '**
  String get wd14FromAnimaTagPresetBody;

  /// No description provided for @animaTagPresetTitle.
  ///
  /// In en, this message translates to:
  /// **'WD14 tags to Anima Tag'**
  String get animaTagPresetTitle;

  /// No description provided for @animaTagPresetBody.
  ///
  /// In en, this message translates to:
  /// **'Convert this dataset\'s WD14 tag captions into Anima Tag captions — one line of tags in the official order, then a period, then a natural-language description.\n\nThe order, one bucket per position: 1 quality (newest, safe), 2 count (1girl, 2boys, no humans), 3 character, 4 series, 5 artist, 6 appearance (hair, eyes, body, clothing, accessories), 7 tags (actions, expressions, composition, viewpoint and everything else — the catch-all), 8 environment (indoors/outdoors, sky, lighting, scenery), then the sentence. Every artist tag must carry an @ prefix — without it the artist tag does almost nothing. Keep quality to newest and safe only; the rest of the quality vocabulary belongs in inference prompts, not in training captions.\n\nSteps:\n1. Run get_dataset_overview and confirm a caption type whose format is animaTag exists; if not, stop and tell me to add one in the caption type settings first. Say which type holds the WD14 tags (the source) and which is the Anima Tag type (the target).\n2. Run get_tag_stats and sort every tag into buckets 1-8. Anything you are unsure about goes into 7, the catch-all. Do not invent a character, series or artist name — use only what I give you below or what the tags already say.\n3. If the Anima Tag type IS the active type, so its own files already hold the WD14 tags, convert the whole dataset with the batch tools rather than image by image: one edit_captions call whose rename maps each artist tag to itself with an @ prefix (e.g. \"someArtist\": \"@someArtist\") and whose add is newest and safe with add_index 0, then one sort_captions_everywhere whose priority list is buckets 1-8 flattened in that order with unlisted set to end. A natural-language tail is never a tag, so these tools cannot disturb it.\n4. If the Anima Tag type is a SEPARATE file from the WD14 one, do the same thing with one convert_captions_to_tags call instead: source is the WD14 type, target is the Anima Tag type, priority is buckets 1-8 flattened, rename gives each artist tag its @ prefix, and prepend is newest and safe. Do not loop write_caption_file — the ordering is a dataset-wide rule, not a per-image judgement.\n5. The description comes last, after a period and a space. Write it only for images you have actually looked at with view_image: one or two sentences on subject, pose and setting, in plain prose and with no tag list in it. If you cannot see the images, leave the sentence off and tell me so — a caption without one is still valid Anima Tag.\n6. When you are done, report how many tags landed in each bucket and list everything left in the catch-all so I can check it.\n\nWhat I already know — character: ; series: ; artist: '**
