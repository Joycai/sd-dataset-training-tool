      ...
      AgentTool(
        ...
        handler: (args) async {
          ...
          final threshold = args['threshold'] is num
              ...
              : ai.threshold;
          final ignore =
              ai.ignoreTags.map((t) => t.toLowerCase()).toSet();
          final canonical = {
            for (final f in deps.dataset.allFiles)
              p.canonicalize(f.path): f.path,
          };
          ...
          for (final rel in paths) {
            ...
            try {
              final resp = await ai.service.interrogateImageFile(
                ...
                File(key),
                models: [
                  AiModelRequest.wd(modelName: model, threshold: threshold),
                ],
              );
              ...
              final tags = resp.allTags
                  .map((t) => AiTaggerService.normalizeTag(
                        t,
                  ...
                  .where((t) =>
                      t.isNotEmpty && !ignore.contains(t.toLowerCase()))
                  .toList();
